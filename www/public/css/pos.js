$(document).ready(function() {
    ...
    $('button.pos-amana-finalize').click(function() {
...
function valider2(){
			...
			if(response.error){
                ...
                if(!response.data){
                    toastr.options = {
                     ...
                     toastr.warning('Se produit ne se trouve pas');
                     $('#product_name_ref').val('');
                    return; 
                }
                $('tbody').append("<tr class='product_row' data-row_index='1'><td>"
                +"<div title='Modifier le produit Prix unitaire et taxe'>"
        +"<span class='text-link text-info cursor-pointer' data-toggle='modal' data-target='#row_edit_product_price_modal_1'>"+response.data.name +"&nbsp;"
            +"<i class='fa fa-info-circle'></i>"
        +"</span>"
        
        
        +"<input type='hidden' name='products[][name]' value='"+response.data.name+"'>"
        +"<input type='hidden' name='products[][price]' value='"+response.data.price+"'>"
        +"</div>"+
                "<input type='hidden' class='enable_sr_no' value='0'>"+
        "<input type='hidden' class='product_type' name='products[1][product_type]' value='single'>"+
    ...
    +"<div class='modal-content'>"+
      +"  <div class='modal-header'>"+
       ...
       +  "   <h4 class='modal-title' id='myModalLabel'>125 - 0122 </h4>"
      + " </div>"
       + "<div class='modal-body'>"
        ...
        +      "          <input type='text' name='products[1][unit_price]' class='form-control pos_unit_price input_number mousetrap valid' value='30.00' aria-invalid='false'>"+
        +      "  </div>"
        +        
        +      "                  <div class='form-group col-xs-12 col-sm-6'>"
        +       "     <label>Type de remise</label>"
        +       "         <select class='form-control row_discount_type' name='products[1][line_discount_type]'><option value='fixed' selected='selected'>Fixé</option><option value='percentage'>Pourcentage</option></select>"
        +       "                     </div>"
        +       " <div class='form-group col-xs-12 col-sm-6'>"+
        ...
        +        "        <input class='form-control input_number row_discount_amount' name='products[1][line_discount_amount]' type='text' value='0.00'>"
         ...
         +         "  <select class='form-control tax_id' name='products[1][tax_id]''><option selected='selected' value=''>Select</option><option value='' selected='selected'>Aucun</option></select>"
          +     " </div>"
          +       "                               <div class='form-group col-xs-12'>"
          +       "     <label>La description</label>"
          +       "                           <textarea class='form-control' name='products[1][sell_line_note]' rows='3'></textarea>"
          +       "     <p class='help-block'>Ajouter le produit IMEI, le numéro de série ou d'autres informations ici.</p>"
          +       " </div>"
          + " </div>"
      + " </div>"
...
+"</div>		</div> </td>"
      ...
      +  "<td class='hide'>"
      +  "<input type='text' name='products[1][unit_price_inc_tax]' class='form-control pos_unit_price_inc_tax input_number' value='30.00'>"
  + " </td>"
   +" <td class='text-center v-center'>"
   +        "     <input type='hidden' class='form-control pos_line_total' value='30.00'>"
   +     "<span class='display_currency pos_line_total_text' data-currency_symbol='true'>"+response.data.price +" د.م</span>"
  +  "</td>"
...
function getProducts(){
    ...
    var products ;
                // $.ajax({
                //     method: 'GET',
                //     url: 'https://sedki.xyz/get_products',
    ...
    // fetch('https://sedki.xyz/get_products')
    //     .then(res => res.json())
...
function validerVente(){
    $mainArr =new Array();
    var arrNames = $('input[name="products[][name]').map(function () {
        return this.value; // $(this).val()
    }).get();
    var arrPrices = $('input[name="products[][price]').map(function () {
        return this.value; // $(this).val()
    }).get();
   ...
   console.log(JSON.stringify($mainArr))
    $.ajax({
		method: 'POST',
		url: 'addVente.php',
		data: {prdts:JSON.stringify($mainArr)},
		dataType: 'json',
		success: function(response){
			if(response.error){
                toastr.options = {
             ...
             toastr.success('Votre Vente est validé avec succes');
		...
		success: function(response){
			if(response.error){
               ...
               toastr.options = {
                ...
                toastr.success('Fichier à été crée avec succés');
                setTimeout(() => {
...
function valider(){ //added for sedki
    var name = $('#product_name_ref').val();
    var product_row = $('input#product_row_count').val();
    var location_id = $('input#location_id').val();
    var customer_id = $('select#customer_id').val();
    ...
    $.ajax({
        method: 'GET',
        url: '/sells/pos/get_product_row_from_name/'+name ,
        async: false,
        data: {
            product_row: product_row,
            customer_id: customer_id,
